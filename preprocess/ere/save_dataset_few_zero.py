...
from utils_pre import pair_trigger_template, read_data_from, dataset_prepare_trigger_template, save_trigger_dataset, save_to_jsonl
import logging
import torch
import json
...
def main(args):
    ...
    # novel domain
    trigger_to_ids = {x: y for x, y in zip(event_types, range(len(event_types)))}
    filtered_class = {'business:declarebankruptcy', 'personnel:nominate', 'transaction:transaction'}
    test_types = [x for x in event_types if x.split(':')[0].lower() in {'life', 'personnel', 'transaction'} and x not in filtered_class]
    test_type_ids = [trigger_to_ids[x] for x in test_types]
    train_types = [x for x in event_types if x.split(':')[0].lower() not in {'life', 'personnel', 'transaction'} and x not in filtered_class]
    train_type_ids = [trigger_to_ids[x] for x in train_types]
    ...
    all_y = [4, 5, 7, 8]
    pad_val = [0, 0, -1, config.metadata.event_count+1]
...
if __name__ == '__main__':
    arg_parser = ArgumentParser(
        'Create few-shot learning data for ere')
