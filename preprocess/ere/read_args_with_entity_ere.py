...
def read_arguments_from_ace(ace_path, SEN_SEP='SENT_SEP'):
    ...
    for i in range(len(data)):
        ...
        if this_triggers:
            trigger_pos_pair1.append(
                [min(np.asarray(this_triggers)[:, i]) for i in range(len(np.asarray(this_triggers)[0]))])
        ...
        output.append(
            sum([[list(_sentence)], this_triggers, this_arguments, [], [this_entities], [this_filler], [this_pos]], []))
...
def read_triggers(offsets, trigger_idxs, trigger_type, verbose=False):
    ...
    while i < tok_num:
        ...
        for this_trigger_type, this_trigger_idxs in zip(trigger_type_list, trigger_id_list):
            ...
            # add I- prefix
            if begin_idx_fixed < end_idx_fixed:
                trigger_bio[begin_idx_fixed + 1:end_idx_fixed + 1] = ['I-' + this_trigger_type] * (
...
def read_ere_dir_to_bio(ace_files, output_path, verbose=False, trigger_set__=None):
    """
...
def read_ere_with_entity(ere_dir, output_dir, split_path, trigger_set=None):
    """
    ...
    :param ere_dir: the path to ace2005 data
    ...
    for data in splits:
        ...
        files = [''.join([ere_dir, i.split('/')[-1], '.csv']) for i in files]
        read_ere_dir_to_bio(files, output_path)
...
if __name__ == '__main__':
    ...
    split_path = 'oneIE_v0.4.3/resource/splits/ERE-EN/'
    output = './data/ere_en/train_files_with_value_time/head'
    read_ere_with_entity(path, output, split_path)
