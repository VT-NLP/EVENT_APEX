...
def pack_data_to_trigger_model_joint(batch, max_len=512):
    """
    Prepare data for trigger detection model
    :param batch:
    :param config:
    ...
    """
    # unpack and truncate data
    (sent_ids_long, dataset_id,
     sent_lengths, bert_sentence_lengths,
     bert_tokens, idxs_to_collect_sent, idxs_to_collect_event, pos_tags, event_tags) = batch

    embedding_length = int(torch.max(bert_sentence_lengths).item())
    bert_sentence_in = bert_tokens[:, :embedding_length]
    max_sent_len = min(max_len, int(torch.max(bert_sentence_lengths).item()))
    bert_sentence_in = bert_sentence_in[:, :max_sent_len]
    idxs_to_collect_sent = idxs_to_collect_sent[:, :max_sent_len]
    trigger_tags = event_tags[:, :max_sent_len]
    pos_tags = pos_tags[:, :max_sent_len]

    # send data to gpu
    sent_lengths = torch.sum(idxs_to_collect_sent!=0, dim=-1)
    tmp = [dataset_id, bert_sentence_in, trigger_tags, idxs_to_collect_sent, idxs_to_collect_event, sent_lengths, \
          sent_ids_long, bert_sentence_lengths, pos_tags]
    return [x.long() for x in tmp] + [embedding_length]
...
def pred_to_event_mention(pred, ids_to_triggers, config):
    ret = []
    ret_w_prob = []
    for i in range(config.metadata.event_count):

        if not torch.any(pred[i]>0):
            continue

        temp = torch.cat([torch.tensor([0]).cuda(), pred[i], torch.tensor([0]).cuda()])
        is_event, begin, end = 0, None, None
        for j in range(len(temp)):
            if temp[j]>0 and not is_event:
                begin = j-1
                is_event = 1
            if not temp[j]>0 and is_event:
                end = j-1
                is_event = 0
                ret.append(tuple([ids_to_triggers[i], begin, end]))
                ret_w_prob.append(tuple([ids_to_triggers[i], begin, end, pred[i][end-1].item()]))
    return ret, ret_w_prob


def pred_to_event_mention_novel(pred, ids_to_triggers, novel_ids):
    ret = []
    ret_w_prob = []
    for i in range(len(novel_ids)):
        ...
        for j in range(len(temp)):
            ...
            if not temp[j]>0 and is_event:
                ...
                is_event = 0
                ret.append(tuple([ids_to_triggers[novel_ids[i]], begin, end]))
                ret_w_prob.append(tuple([ids_to_triggers[novel_ids[i]], begin, end, pred[i][end-1].item()]))
    return ret, ret_w_prob